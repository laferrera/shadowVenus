...
uniform float time;
uniform float xDistance = 0.0;
uniform float yDistance = 0.0;
uniform float heightDistance = 0.0;
...
uniform float SEA_HEIGHT = 0.6; //0.6
uniform float SEA_CHOPPY = 4.0; //4.0
uniform float SEA_SPEED = 0.2; //0.8
uniform float SEA_FREQ = 0.10; //0.16
// const vec3 SEA_BASE = vec3(0.1,0.19,0.22);
// const vec3 SEA_WATER_COLOR = vec3(0.8,0.9,0.6);
const vec3 SEA_BASE=vec3(0.2157, 0.102, 0.2196);
const vec3 SEA_WATER_COLOR=vec3(0.8824, 0.902, 0.6);
// float SEA_TIME;
#define SEA_TIME (1.+time*SEA_SPEED)
#define EPSILON_NRM (.1/resolution.x)
...
// float colorfulNoise(in vec3 p){
vec3 colorfulNoise(in vec3 p){    
    vec3 i=floor(p);
    vec3 f=fract(p);
    ...
    float mixF = mix(mix(mix(dot(hash(i+vec3(0.,0.,0.)),f-vec3(0.,0.,0.)),
...
vec3 getSeaColor(vec3 p, vec3 n, vec3 l, vec3 eye, vec3 dist) {  
    ...
    fresnel = pow(fresnel,2.0) * 0.95;
        
    // bteitler: Bounce eye ray off ocean towards sky, and get the color of the sky
    // vec3 reflected = getSkyColor(reflect(eye,n));    
    vec3 reflected=colorfulNoise(reflect(eye,n)*2.+time*.3)*reflect(eye,n).rgb*2.;

    // bteitler: refraction effect based on angle between light surface normal
    // vec3 refracted = SEA_BASE + diffuse(n,l,80.0) * SEA_WATER_COLOR * 0.12; 
    float diffusedF = diffuse(n,l,80.)*2.;
    vec3 diffused = vec3(diffusedF,diffusedF,diffusedF);
    diffused = colorfulNoise(diffused+time*.3)*diffused*2.;
    // vec3 refracted = SEA_BASE + diffused * SEA_WATER_COLOR * 0.12; 
    vec3 refracted = diffused;
...
void main() {
    ...
    // vec3 ori=vec3(0.,3.5,5.);
    // vec3 ori = vec3(0.0,3.5,time*5.0);
    // vec3 ori = vec3(0.0,3.5,depthDistance*2.0 + 5);
    vec3 ori = vec3(0.0+xDistance,3.5+heightDistance,yDistance*2.0 + 5);
    ...
    // vec4 waveColor = vec4(pow(color,vec3(.75)),1.);
    // vec4 waveColor = vec4(pow(color,vec3(.40)),1.);
    vec4 waveColor = vec4(pow(color,vec3(.50)),1.);

    // gl_FragColor.rgb=colorfulNoise(waveColor.rgb*2.+time*.3)*waveColor.rgb*2.;
